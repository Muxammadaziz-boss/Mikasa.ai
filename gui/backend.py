...
class BackendBridge:
    ...
    def init_backend(self):
        ...
        def _init():
            try:
                ...
                # GUI ga xabar
                self.app.after(0, lambda: self.app.set_status("online", "Tayyor"))
                self._add_activity("🟢 Backend ishga tushdi", "success")
                
            except Exception as e:
                logger.error(f"Backend init xatolik: {e}")
                self.app.after(0, lambda: self.app.set_status("offline", f"Xatolik: {e}"))
    ...
    def send_text_command(self, text):
        ...
        # Status yangilash
        self.app.after(0, lambda: self.app.set_status("busy", "Ishlamoqda..."))
    ...
    def start_listening(self):
        ...
        self.app.after(0, lambda: self.app.set_status("listening", "Tinglayapman..."))
    ...
    def _gui_callback(self, xabar):
        ...
        Thread-safe — app.after() ishlatadi.
        """
        def _update():
            try:
                ...
                if is_user_speech:
                    ...
                    self._add_activity(
                        xabar[:60], 
                        "success" if "✅" in xabar else "info"
                    )
                    
            except Exception as e:
                logger.debug(f"GUI callback xatolik: {e}")
        
        # Thread-safe yangilash
        try:
            self.app.after(0, _update)
        except Exception:
            pass  # App yopilgan bo'lishi mumkin
    ...
    def _add_activity(self, text, category="info"):
        ...
        try:
            self.app.after(0, self._refresh_dashboard_activity)
        except Exception:
            pass
    ...
    def _refresh_dashboard_activity(self):
        ...
        try:
            ...
            for act in self._activities[:8]:
                row = ctk.CTkFrame(
                    dashboard._activity_list, fg_color="transparent"
                )
                row.pack(fill="x", pady=2)
                
                ctk.CTkLabel(
                    row, text="●", font=(Fonts.FAMILY, 8),
                    text_color=act["color"], width=14
                ).pack(side="left", padx=(0, 6))
                
                ctk.CTkLabel(
                    row, text=act["text"], font=Fonts.SMALL,
                    text_color=Colors.TEXT_PRIMARY, anchor="w"
                ).pack(side="left", fill="x", expand=True)
                
                ctk.CTkLabel(
                    row, text=act["time"], font=Fonts.TINY,
