...
try:
    from ai_engine import ai_savol_yuborish, ai_mavjudmi, suhbat_tarixini_tozalash, ekran_tahlil, ekran_element_top, buyruq_tekshir
    AI_AVAILABLE = True
except ImportError:
    AI_AVAILABLE = False
...
try:
    from core.smart_algorithms import (
        algoritmlarni_tayyorla, aqlli_buyruq_aniqla, keyingi_bashorat,
        get_cache, get_bashorat, get_gemini_limiter, get_openrouter_limiter,
        statistika as algo_statistika, xavfli_buyruqmi, buyruq_prioriteti,
        XAVFLI_BUYRUQLAR
    )
...
def kayfiyat_aniqla(matn):
    ...
    # Kayfiyat so'zlari
    XURSAND = ["ajoyib", "zo'r", "yaxshi", "barakalla", "tabriklayman", "salom", "rahmat", 
               "marhamat", "qoyil", "chiroyli", "sevaman", "ishga tushdi", "tayyor", "ochildi",
               "muvaffaqiyat", "ishladi", "to'g'ri", "ha!", "super", "shunday"]
    GAMGIN = ["kechirasiz", "uzr", "afsuski", "xato", "muammo", "topilmadi", "ishlamadi",
              "buzildi", "yo'q", "olmadi", "xatolik", "muvaffaqiyatsiz", "qiyin"]
    HAYAJON = ["voy", "qara", "tezroq", "shoshiling", "muhim", "diqqat", "ehtiyot",
               "tez", "iltimos", "shart", "darrov", "keling"]
    JIDDIY = ["eslatma", "ogohlantirish", "xavfsizlik", "parol", "o'chirish", "qulflash",
              "o'chirildi", "yopildi", "to'xtatildi", "bloklandi"]
    ILIQ = ["yoqimli", "salqin", "sayir", "dam ol", "choy", "kurtka", "issiq",
            "havo", "daraja", "ob-havo", "bulutli", "quyosh", "yomg'ir"]
    XAVOTIR = ["sovuq", "momaqaldiroq", "kuchli", "bo'ron", "ehtiyot bo'ling",
               "soyabon", "iliq kiyining", "juda issiq", "juda sovuq"]
    
    # Kayfiyatni aniqlash
    ball = {"xursand": 0, "gamgin": 0, "hayajon": 0, "jiddiy": 0, "iliq": 0, "xavotir": 0}
    
    for soz in XURSAND:
        if soz in matn_kichik: ball["xursand"] += 1
    for soz in GAMGIN:
        if soz in matn_kichik: ball["gamgin"] += 1
    for soz in HAYAJON:
        if soz in matn_kichik: ball["hayajon"] += 1
    for soz in JIDDIY:
        if soz in matn_kichik: ball["jiddiy"] += 1
    for soz in ILIQ:
        if soz in matn_kichik: ball["iliq"] += 1
    for soz in XAVOTIR:
        if soz in matn_kichik: ball["xavotir"] += 1
...
def ovoz_chiqar_tez(text):
    """Tez ovozli javob — Silero TTS (local) → Edge TTS (fallback)"""
    def _ijro_et():
        try:
            # Mikrofonni o'chirish — feedback loop oldini olish
            global_state.gapirmoqda = True
            logging.debug(f"Audio playing request: {text}")
            
            # 1-USUL: TTS Manager (Silero → Edge TTS avtomatik)
            try:
                from core.tts_manager import get_tts_manager
                tts = get_tts_manager(global_state.ovoz_turi_global)
                
                # Kayfiyatga mos parametrlar (Edge TTS fallback uchun)
                rate, pitch, volume = kayfiyat_aniqla(text)
                kayfiyat_params = {"rate": rate, "pitch": pitch, "volume": volume}
                
                # Ovoz yaratish (Silero birinchi, Edge fallback)
                audio_file = tts.speak(text, kayfiyat_params)
                if audio_file:
                    tts.play_audio(audio_file)
                    logging.debug(f"TTS ({tts.engine_name}) muvaffaqiyatli")
                    return
            except ImportError:
                logging.debug("TTS Manager topilmadi, Edge TTS ishlatiladi")
            except Exception as e:
                logging.warning(f"TTS Manager xatolik: {e}")
            
            # 2-USUL: Legacy Edge TTS (agar TTS Manager ishlamasa)
            if not TTS_AVAILABLE:
                msg = f"TTS (Ovoz) not available: {text}"
                print(msg)
                logging.warning(msg)
                return
            ...
            async def speak():
                try:
                    logging.debug("Starting legacy Edge TTS")
                    ...
                    logging.error(f"Edge TTS xatolik: {e}")
                    print(f"TTS xatolik: {e}")
            
            # asyncio.run() — yangi loop yaratib, tugagach yopadi (leak yo'q)
            try:
                asyncio.run(speak())
            except RuntimeError:
                # Agar mavjud loop bo'lsa — fallback
...
def tingla():
    ...
    try:
        ...
        # sd.wait() o'rniga, tinglash_faol ni tekshirib turish
...
def chrome_oyna_yop_aqlli():
    ...
    try:
        ...
        ovoz_chiqar_tez(f"Oxirgi Chrome oynasini yopaveraymi? U yerda '{tab_nomi[:30]}' ochiq. Ko'rishni xohlamaysizmi?")
...
SHAHAR_NOMLARI = {
    ...
    "farg'ona": "Fergana", "parvona": "Fergana", "farg'onada": "Fergana",
    "navoiy": "Navoi", "nukus": "Nukus", "qarshi": "Karshi",
    "jizzax": "Jizzakh", "termiz": "Termez", "urganch": "Urgench",
    "xorazm": "Khiva", "qo'qon": "Kokand", "marg'ilon": "Margilan",
    "chirchiq": "Chirchik", "olmaliq": "Almalyk", "guliston": "Gulistan",
    "denov": "Denau", "shahrisabz": "Shahrisabz", "zarafshon": "Zarafshan",
    "tashkent": "Tashkent", "fergana": "Fergana", "samarkand": "Samarkand",
    # Boshqa mamlakatlar
    "moskva": "Moscow", "istanbul": "Istanbul", "dubai": "Dubai",
    "seul": "Seoul", "tokio": "Tokyo", "london": "London",
}
...
def ob_havo_olish(shahar="Tashkent"):
    ...
    HAVO_TARJIMA = {
        "clear sky": "toza osmon", "few clouds": "oz-moz bulutli",
        "scattered clouds": "bulutli", "broken clouds": "bulutli",
        "overcast clouds": "to'liq bulutli", "light rain": "yengil yomg'ir",
        "moderate rain": "o'rtacha yomg'ir", "heavy intensity rain": "kuchli yomg'ir",
        "light snow": "yengil qor", "snow": "qor yog'moqda",
        "mist": "tumanli", "fog": "tumanli", "haze": "tumanli",
        "thunderstorm": "momaqaldiroq", "drizzle": "mayda yomg'ir",
    }
    
    # O'zbek shahar nomlarini qaytarish (API dan kelgan nomni o'zbekchaga)
    SHAHAR_UZB = {
        "Fergana": "Farg'ona", "Tashkent": "Toshkent", "Samarkand": "Samarqand",
        "Bukhara": "Buxoro", "Andijan": "Andijon", "Namangan": "Namangan",
        "Navoi": "Navoiy", "Nukus": "Nukus", "Karshi": "Qarshi",
        "Jizzakh": "Jizzax", "Termez": "Termiz", "Urgench": "Urganch",
        "Kokand": "Qo'qon", "Margilan": "Marg'ilon", "Chirchik": "Chirchiq",
        "Gulistan": "Guliston", "Denau": "Denov",
    }
    ...
    try:
        ...
        gui_matn = (f"{emoji} {shahar_nomi}: {harorat}°C, {holat_uz}. "
        ...
        return None, "Kechirasiz, ob-havo serveri javob bermadi. Birozdan keyin qaytadan urinib ko'ring"
    except Exception as e:
...
def musiqa_qidir(query, platform="youtube"):
    try:
        ...
        def _auto_play_music():
            if platform == "yandex":
                # Yandex Music animatsiya bor — 8 sekund kutish + 3 marta urinish
                time.sleep(8)
                try:
                    for i in range(3):
                        pyautogui.press('space')
                        logging.debug(f"Auto-play: Yandex Music Space bosildi (urinish {i+1})")
                        time.sleep(3)
                except Exception as e:
...
UZBEK_RAQAMLAR = {
    ...
    "besh": 5, "olti": 6, "yetti": 7, "sakkiz": 8, "toqqiz": 9, "to'qqiz": 9,
    "on": 10, "o'n": 10, "yigirma": 20, "ottiz": 30, "o'ttiz": 30,
    "qirq": 40, "ellik": 50, "oltmish": 60, "yetmish": 70,
    "sakson": 80, "tukson": 90, "to'qson": 90, "yuz": 100
}
...
def buyruqni_aniqla(matn):
    ...
    if ovoz_bor:
        ...
        if raqam is not None and raqam > 0:
            ...
            oshir_bor = any(s in matn_toza for s in ["oshir", "kotar", "kottar", "baland"])
            pasayt_bor = any(s in matn_toza for s in ["pasayt", "kamayt", "past", "tushir"])
        ...
        if any(s in matn_toza for s in ["oshir", "kotar", "kottar", "baland", "kuchli"]):
            return ("volume_up", 10)
        
        # "ovozni pasayt", "ovozni kamaytir", "ovoz past"
        if any(s in matn_toza for s in ["pasayt", "kamayt", "past", "tushir", "kichik", "sekin"]):
    ...
    AGENT_GA_YUBORISH_KALITLAR = {
        "date": ["dollar", "kurs", "valyuta", "narx", "pul", "som", "rubl", "yevro", "euro", "bitcoin"],
...
def _agent_init():
    ...
    try:
        from ai_engine import agent_ai_call
        ...
        def agent_step_callback(step_num, step_type, data):
            if step_type == "thought":
                gui_ga_xabar_yuborish(f"🧠 Qadam {step_num}: {data.get('thought', '')}")
            elif step_type == "tool_result":
                tool = data.get('tool', '')
                result = data.get('result', {})
                msg = result.get('result', {}).get('message', '') if isinstance(result.get('result'), dict) else str(result)[:100]
...
def agent_pipeline_run(matn):
    ...
    if result.get("success"):
        ...
        # Natijani ko'rsatish
        if tools_used:
            gui_ga_xabar_yuborish(f"✅ Agent: {response} (tool'lar: {', '.join(tools_used)})")
        else:
...
def openrouter_ai_suhbat(matn):
    ...
    if SMART_ALGO_AVAILABLE:
        ...
        if not ruxsat:
            gui_ga_xabar_yuborish(f"⚠️ AI so'rovlar limiti tugadi. {qoldiq}s kuting.", ovoz=True)
            return
    ...
    if javob is None:
        gui_ga_xabar_yuborish("❌ AI javob bermadi. Internet aloqasini tekshiring.", ovoz=True)
        return
    ...
    if javob.get("type") == "command":
        ...
        # Xavfli buyruqlarni tasdiqlash
        if SMART_ALGO_AVAILABLE and xavfli_buyruqmi(intent):
            gui_ga_xabar_yuborish(f"⚠️ AI xavfli buyruq: {intent}. Tasdiqlaysizmi?")
            ovoz_chiqar_tez(f"Diqqat! {response_text} — tasdiqlaysizmi?")
            javob_tasdiqlash = tingla()
            if not javob_tasdiqlash or not any(s in javob_tasdiqlash.lower() for s in ["ha", "tasdiql", "bajar", "qil"]):
    ...
    try:
        ...
        # ========== Ilova ochish ==========
            ...
            try:
                ...
                # code PATH da yo'q — shell orqali urinib ko'rish
        ...
        # ========== Salomlashish / identifikatsiya ==========
            ...
            ism = foydalanuvchi_ismi or "Do'stim"
            gui_ga_xabar_yuborish(f"🤖 Men Mikasa AI yordamchiman! {ism} uchun yaratilganman.")
            ovoz_chiqar_tez(f"Men Mikasa, sun'iy intellektga ega shaxsiy yordamchiman. "
        ...
        # ========== Ob-havo ==========
            ...
            if not shahar and matn:
                ...
                for soz in ["havo", "ob-havo", "harorat", "qanaqa", "qanday", "hozir", "bugun", "necha", "daraja", "gradus"]:
        ...
        # ========== Eslatmalar ==========
            ...
            eslatma_matni = params.get("text", "") or params.get("query", "") or params.get("reminder", "")
        ...
        # ========== AI suhbat rejimi ==========
            ...
            if not AI_AVAILABLE:
                ...
                while global_state.tinglash_faol:
                    savol = tingla()
                    if savol:
                        if any(s in savol.lower() for s in ["chiq", "to'xta", "yetadi", "bo'ldi", "rahmat"]):
                            ovoz_chiqar_tez("AI rejimdan chiqdim. Buyruqlaringizni kutaman.")
                            break
...
def _musiqa_search_matn(matn):
    ...
    filtr_sozlar = [
        "musiqa", "qoshiq", "qoy", "ijro", "et", "play",
        "youtube", "yutub", "yutuq", "yandex", "yandeks", "spotify",
        "eshit", "eshitaylik", "eshitamiz", "esla", "eslaylik",
        "tingla", "tinglaylik", "tinglaymiz",
        "qoyaylik", "qoyamiz",
        "dan", "da", "ni", "ga", "qidir", "och", "yoq",
        "salom", "menga", "yangi", "mening", "mikasa",
        "ber", "kerak", "iltimos", "bir",
    ]
    ...
    if query:
        ...
        platform_nomi = {"youtube": "YouTube", "yandex": "Yandex Music", "spotify": "Spotify"}
...
def buyruqni_tushun(matn, foydalanuvchi_ismi, ovoz_turi):
    logging.debug(f"DEBUG: buyruqni_tushun called with: {matn}")
    try:
        with global_state.buyruq_bajarilmoqda_lock:
            global_state.ovoz_turi_global = ovoz_turi
            buyruqni_saqla(matn)
        
        logging.debug("DEBUG: Lock acquired and released")
        
        # Aqlli algoritmlar bilan aniqlash
        if SMART_ALGO_AVAILABLE:
            intent = aqlli_buyruq_aniqla(matn, buyruqlar_json, regex_fallback_func=buyruqni_aniqla)
        else:
            intent = buyruqni_aniqla(matn)
        logging.debug(f"DEBUG: Intent detected: {intent}")
        
        # Tuple (buyruq, qiymat)
        if isinstance(intent, tuple):
            buyruq, qiymat = intent[0], intent[1]
            logging.debug(f"DEBUG: Handling tuple intent: {buyruq}, {qiymat}")
            
            if buyruq == "confirm":
                score = intent[2] if len(intent) > 2 else 0
                gui_ga_xabar_yuborish(f"🤔 {qiymat} (TF-IDF {score:.2f}) - aniq emas")
                # Agent orqali tasdiqlash so'raymiz
                agent_pipeline_run(f"Men '{matn}' dedim. Balki '{qiymat}' degan buyruqni nazarda tutgandirman? Mendan shu buyruqni bajarishni xohlayotganimni ask_user orqali tasdiqlashimni so'ra.")
                return
            elif buyruq == "volume_set":
                ovoz_sozlash(qiymat)
        ...
        # Xavfli buyruqlarni tekshirish
        if SMART_ALGO_AVAILABLE and xavfli_buyruqmi(intent):
            logging.warning(f"Xavfli buyruq ushlandi: {intent}")
            gui_ga_xabar_yuborish(f"⚠️ XAVFLI BUYRUQ: {intent}")
            agent_pipeline_run(f"Men '{matn}' orqali '{intent}' qilmoqchi bo'ldim. Bu xavfli. Mendan ask_user orqali rostdan ham '{intent}' ni bajarishni xohlayotganimni tasdiqlashimni so'ra va ha desa bajar, yo'q desa bekor qil.")
            return
        
        # Keyingi buyruq bashorati
        if SMART_ALGO_AVAILABLE:
            bashoratlar = keyingi_bashorat()
            if bashoratlar:
                eng_ehtimoliy = bashoratlar[0]
                if eng_ehtimoliy[1] >= 0.5:  # 50% dan yuqori ehtimol
                    logging.debug(f"Bashorat: keyingi → {eng_ehtimoliy[0]} ({eng_ehtimoliy[1]*100:.0f}%)")
        
        gui_ga_xabar_yuborish(f"🎯 Buyruq: {intent}")
        
        # Buyruqlarni bajarish — _intent_bajar() ga delegatsiya
        bajarildi = _intent_bajar(
            intent, 
            params=None, 
            matn=matn, 
            foydalanuvchi_ismi=foydalanuvchi_ismi
        )
        
        if not bajarildi:
            # Agent Pipeline — _intent_bajar tushunmagan buyruqni Agent ga yuborish
            logging.debug(f"Intent topilmadi, Agent ga yuborilmoqda: {matn}")
            gui_ga_xabar_yuborish(f"🤖 Agent tushunishga harakat qilmoqda...")
    ...
    def patched_init(self, *args, **kwargs):
        ...
        def wrapped_run(*a, **kw):
            try:
                ...
                logging.error(f"Thread '{self.name}' xatolik: {e}", exc_info=True)
